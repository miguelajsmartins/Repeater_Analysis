...
for i in range(n_iter):
    ...
    #accept events according to the exposure
    ...
    print('Accepting events for %i samples took' % n_samples, start_2 - start_1, ' s')

    #save samples as dataframes
    for sample in range(n_samples):
        ...
        #drop nan values and accept only n_accept events
        ...
        #save dataframe
        counter+=1
        accepted_event_data.to_parquet(os.path.join(output_path, 'IsoDist_%i_acceptance_th80_%s_%s_sample_%i.parquet' % (int(n_accept), start_date_fits, end_date_fits, counter)), index=True)

    print('Cleaning and saving samples took', datetime.now() - start_2, ' s')

print('Treating all samples took', datetime.now() - start_all, 's')
