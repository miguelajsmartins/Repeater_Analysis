...
#calculate the kernel for the lambda values below the fit limit
def get_gaussian_kernel(fit_initial, lambda_array):

    #estimate the pdf of lambda using a gaussian kernel density
...
def get_lambda_pvalue(index, lambda_dist, lambda_array):
    ...
    kernel_pdf = get_gaussian_kernel(fit_initial, lambda_array)
    ...
    #
    #lambda_pvalues = 1 - np.array([kernel_pdf.integrate_box_1d(cdf_bin_centers[0], lambda_value) for lambda_value in lambda_array[lambda_below_fit_init]])
    ...
    #lambda_pvalues[lambda_above_fit_init] = (1 - cdf_bin_content[cdf_bin_centers < fit_initial][-1])*np.exp(-tail_slope*(lambda_array[lambda_above_fit_init] - fit_initial))
