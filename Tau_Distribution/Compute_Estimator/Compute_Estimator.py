...
path_to_dir_ud = '/home/miguelm/Documents/Anisotropies/Repeater_Analysis/DataSets/UD_large_stats/'
...
ax_tau_log.hist(np.log10(tau_repeater),bins=200, range=[-5,4], alpha = 0.5, label=r'Isotropy + ' + N_ACCEPTED_REP_EVENTS + r' events from repeater w/ $\tau = 1$ hour')

log_tau_avg_hist_edges, log_tau_avg_hist_content = AverageTauDist(list_of_log_tau_arrays, 200, -5, 4)
...
fig_tau_log.savefig('./results/Average_log10tau_distribution_wRepeater_RandPos_' + PERIOD_OF_REP + '.pdf')
...
ax_tau.hist(tau_repeater,bins=200, range=[0,10], alpha = 0.5, label=r'Isotropy + ' + N_ACCEPTED_REP_EVENTS + r' events from repeater w/ $\tau = 1$ hour')
...
cdf_rep_bin_edges, cdf_rep_content = ComulativeDistFunc(np.log10(tau_repeater), 200)
cdf_ud_bin_edges, cdf_ud_content = ComulativeDistFunc(np.log10(tau_ud_all), 200)

ax_cdf_tau_log.plot(cdf_ud_bin_edges, cdf_ud_content, label=r'Isotropy')
ax_cdf_tau_log.plot(cdf_rep_bin_edges, cdf_rep_content, label=r'Isotropy + ' + N_ACCEPTED_REP_EVENTS + r' events from repeater w/ $\tau = 1$ hour')

ax_cdf_tau_log.set_title(r'$N(\log_{10}(\tau))$ for angular window $\Delta \theta = 1^\circ$', fontsize=24)
...
fig_cdf_tau_log.savefig('./results/Average_log10tau_CDF_wRepeater_RandPos_' + PERIOD_OF_REP + '.pdf')
...
for i in range(1,len(list_of_integration_lims)):
    ...
    ax_est.legend(loc='upper left', fontsize=18)

    fig_est.savefig('./results/Estimator_distribution_histogram_RandPos_' + PERIOD_OF_REP + '.pdf')
