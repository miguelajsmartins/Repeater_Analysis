...
#generate repeater with period and lifespan in seconds
def Generate_Exponential_Repeater(n_events, delta, alpha, time_stamp, avg_period, lat_pao, long_pao):
    ...
    for i in range(n_events):
        ...
        alpha_gauss = np.random.normal(alpha, math.radians(0.1), 1)[0]
        delta_gauss = np.random.normal(delta, math.radians(0.1), 1)[0]

        event_list.append(Event(alpha_gauss,delta_gauss,time,math.nan))
...
rep_nevents = 1000
rep_alpha = math.radians(120)
rep_delta = math.radians(-30)
rep_period = 86164 #one sidereal day in seconds
rep_date = '2015-01-01T00:00:00'

#generate repeater events
repeater_events = Generate_Exponential_Repeater(rep_nevents, rep_delta, rep_alpha, rep_date, rep_period, lat_pao, long_pao)
...
for evt in repeater_events:
    ...
    if AcceptEvent(evt.time.gps, gps_time, nT5, nT5_max, lat_pao, theta_min, theta_max, evt.delta, evt.alpha, sidereal_time):
...
print('Number of events =', N_gen_events,' Number of accepted events =', len(accepted_ud_events),' Percentage of events kept',eff,'%')
...
data.to_parquet('./output/TimeOrdered_Events_ExponentialRepeater_Date_' + rep_date + '_Period_' + str(rep_period)+ '_TotalEvents_' + str(N_events) + '_AcceptedRepEvents_' + str(N_accepted_rep_events) + '.parquet', index=False)
...
output_rep_data.to_parquet('./output/Rep_events_with_tau_Date_' + rep_date + '_Period_' + str(rep_period)+ '_TotalEvents_' + str(N_events) + '_AcceptedRepEvents_' + str(N_accepted_rep_events) + '.parquet', index=False)
...
output_path = '/home/miguelm/Documents/Anisotropies/DataSets/MockData_Repeaters/'
data.to_parquet(output_path + 'TimeOrdered_Events_ExponentialRepeater_Date_' + rep_date + '_Period_' + str(rep_period)+ '_TotalEvents_' + str(N_events) + '_AcceptedRepEvents_' + str(N_accepted_rep_events) + '.parquet', index=False)
